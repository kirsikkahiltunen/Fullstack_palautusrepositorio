...
describe('most blogs', () => {
    ...
    test('when list has only one blog function returns information of author of this blog', () => {
      const result = listHelper.mostBlogs(blogListOne)
      assert.deepStrictEqual(result, { author: 'Laura Korhonen', blogs: 1 })
    })

    test('when list have more than one blog, the function returns the author with most blogs', () => {
        const result = listHelper.mostBlogs(blogListTwo)
        assert.deepStrictEqual(result, { author: 'Jari Salmi', blogs: 2 })
    })
